    ...
    for i in tqdm(range(n)):
        ...
        while machine.get_triggers(model, machine.get_model_state(model).name) != []:
            ...
            if "silent" not in activity:
                ...
                traces.append([i, activity.get_label(), activity.get_high_level_activity().get_label(),role, resource, starttime, timestamp, model.get_attribute_value("isComplete"), model.get_attribute_value("isAccepted")])
        ...
        shift_start, shift_end = time_ress.get_user_shift(role)
        casestamp = simulation.check_time(datum=casestamp, shift_start=shift_start, shift_end=shift_end)
        timestamp = simulation.check_time(timestamp, shift_start=shift_start, shift_end=shift_end)
    return traces

#not generic part: need to decide what happend with the different attributes and needed to be added to a list 
