...
def main(config):
    ...
    if loaded:
        if len(pd_events_fv) > 100000:
            ...
            to_remove = []
            for f in filtered:
                if len(pd_events_fv[pd_events_fv[XES_CASE] == int(f.replace("Application_", ""))]) != len(pd_log[pd_log[XES_CASE] == f]):
        ...
        if config.multi_clustering:
            for clustering_num, pred_labels in clust.pred_labels.items():
                ...
                pd_events_fv[CLUST_COL + str(clustering_num)] = pd_events_fv[CLUST_COL + str(clustering_num)].astype(int)
        ...
        clust_to_att_unique, clust_to_att_distinct, clust_to_att_distinct_per_case = ranker.create_ranking()
        toc = time.perf_counter()
        print(f"Props computed in {toc - tic:0.4f} seconds")
        # print(len(props.clust_to_prop))
        # print(props.clust_to_prop)
        # Generate cluster descriptions
        text_gen = TextGen(log, props.clust_to_prop, clust_to_att_unique, clust_to_att_distinct, clust_to_att_distinct_per_case, config)
...
def evaluate():
    ...
    for raw_config in raw_configs:
        ...
        if conf.log_name == BPI17_NAME and conf.noise_tau == 0 and conf.dim_red==PCA_S and conf.comp == 0.99:# or conf.log_name == BPI17_NAME and conf.noise_tau == 0 and conf.dim_red==TSNE_S and conf.comp == 2 or conf.log_name == BPI17_NAME and conf.noise_tau == 0.2 and conf.dim_red==TSNE_S and conf.comp == 0.95:
            main(conf)
        #break
